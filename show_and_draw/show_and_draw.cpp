...
vector<Member> initialize_members(string nation){
	...
	if(!fin.is_open()){
		...
		for(int i=0; i<5; i++){
			...
			for(int j=0; j<n; j++){
				...
				team.set_WR(rank);
				team.set_members(initialize_members(nation)); 
...
void draw(){
	...
	for(int i=0; i<32; i++){
		if(teams[i].get_WR()==65){
			...
			else if(teams[i].get_WR()>=8&&teams[i].get_WR()<=17){
				pot[1].push_back(teams[i]);
			}
			else if(teams[i].get_WR()>=18&&teams[i].get_WR()<=34){
